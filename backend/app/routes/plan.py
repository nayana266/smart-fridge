...
from ..shared.models.plan import PlanResponse, InventoryItem, SwapSuggestion, LLMContext
...
def plan(
    ...
    for item in items:
        # support passing either a raw string name or a dict from vision
        ...
        entry = semantic_lookup(name)
        impact = entry["tag"]
        category = entry.get("category", "other")
        co2e_100g = entry.get("co2e_100g")

        inventory.append(InventoryItem(
            name=name, count=1, confidence=confidence, impact=impact, category=category, co2e_100g=co2e_100g
        ))

        # Generate intelligent swaps for medium/high impact items
        if impact in ["medium", "high"]:
            to_item, why = suggest_swap(name, category, co2e_100g)
            if to_item:
